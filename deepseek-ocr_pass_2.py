...
# === CONFIGURATION ===
PDF_PATH = Path("./MSFT_FY25q4_10K.pdf")  # Updated to local path
...
def ollama_generate(image: Image.Image, prompt: str, timeout: int = 90) -> Dict[str, Any]:
    """
    ...
    resp = requests.post(OLLAMA_BASE_URL, json=payload, timeout=timeout)
...
""".strip(),
        ...
        for label, prompt in prompts:
            ...
            if time.time() - table_start_time > max_table_time:
                print(f" Timeout reached for this table ({max_table_time}s), skipping to next table.")
                break
            ...
            if is_likely_complete_table(new_table_md, orig_md):
                ...
                if table_shape(new_table_md)[0] >= table_shape(orig_md)[0] and \
                   table_shape(new_table_md)[1] >= table_shape(orig_md)[1]:
                    ...
                    break
            # Check timeout after each prompt attempt for full page
            if time.time() - table_start_time > max_table_time:
                print(f" Timeout reached for this table ({max_table_time}s), skipping to next table.")
                break
        ...
        if best_full_page_result:
            ...
            for variant_label, variant_image in image_variants:
                ...
                if crop_attempts >= max_crop_attempts:
                    print(f" Maximum crop attempts reached ({max_crop_attempts}), stopping.")
                    break
                # Check timeout before processing each crop
                if time.time() - table_start_time > max_table_time:
                    print(f" Timeout reached for this table ({max_table_time}s), skipping to next table.")
                    break
                ...
                for label, prompt in prompts:
                    ...
                    if is_likely_complete_table(new_table_md, orig_md):
                        ...
                        ocr_success = True
                        break
                if ocr_success or (time.time() - table_start_time > max_table_time):
                    break
                crop_attempts += 1
